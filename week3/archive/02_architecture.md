...
**Single Job:** Run the daily check-in loop. Detect deviations from expected recovery trajectory. Classify each check-in as GREEN / YELLOW / RED.

**Tools:**
- Pinecone (retrieve patient history + care plan)
- Claude claude-sonnet-4-6 (symptom assessment + classification)
- Streamlit session (render check-in UI)
...
Step 12 Patient completes check-in
...
Step 20 On Day 30, Admin Agent generates final recovery summary
Step 21 Summary formatted for patient to share with primary care physician
```
...
| Daily check-ins | Responses, classifications, timestamps | Pinecone | Persistent, queried for trends |
...
> Live demo: upload a synthetic discharge PDF → care plan generated in under 2 minutes → simulated Day 7 check-in with chest pain → escalation pathway triggered → provider message drafted → human approval UI displayed. End to end in under 5 minutes on stage.
...
**Agent Logic:** Python (LangChain or direct Anthropic SDK calls per agent)
**LLM:** Claude claude-sonnet-4-6 via Anthropic API
**Vector Store:** Pinecone (patient state, discharge content, check-in history)
**UI:** Streamlit (upload portal, care plan dashboard, check-in form, approval queue)
